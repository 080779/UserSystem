...
namespace IMS.Service.Service
{
    public class OrderService : IOrderService
    {
        ...
        public async Task<long> AddAsync(long buyerId, string orderCode, decimal amount, decimal profit,int zone)
        {
            bool result = false;
            string buyerMobile = string.Empty;
            long orderId = 0 ;
            decimal aPrice = await cashHallService.GetExchangeAsync();
            decimal currentValue = profit / aPrice;

            using (MyDbContext dbc = new MyDbContext())
            {
                OrderEntity entity = new OrderEntity();
                entity.Code = orderCode;
                entity.BuyerId = buyerId;
                entity.AddressId = 123;//addressId;
                entity.PayTypeId = 11;// payTypeId;
                entity.OrderStateId = 20;// orderStateId;
                entity.Amount = amount;
                entity.DiscountAmount = profit; //支付的积分
                entity.PostFee = aPrice;//1A积分=xxx人民币
                entity.DeductAmount = currentValue; //实际支付的A积分
                entity.UpAmount = zone;  //1：会员专区，0：非会员专区
                dbc.Orders.Add(entity);

                UserEntity userEntity = await dbc.GetAll<UserEntity>().SingleOrDefaultAsync(g => g.Id == buyerId);
                if(zone==1)
                {
                    userEntity.PersonalScore += amount;  //订单金额累计
                }
                buyerMobile = userEntity.Mobile;
            ...
            {
                //静态奖、消费送积分,直推奖
                result = await bonusService.CalcStaticAward(orderId, orderCode, buyerId);
                log.DebugFormat($"静态奖、消费送积分，直推奖 CalcStaticAward:{result},orderCode:{orderCode}，buyerId：{buyerId}");
            }
            else if (currentValue > 0 && zone == 0)
            {
                //直推奖改在 静态奖,消费送积分 结算
                //直推奖
                //await bonusService.CalcAwardRecommend(orderId, orderCode, currentValue, buyerId);

                //购物奖、累计业绩
                result = await bonusService.CalcAwardBuy(orderId, orderCode, currentValue, buyerId);
                log.DebugFormat($"购物奖、累计业绩 CalcAwardBuy:{result},orderCode:{orderCode}，buyerId：{buyerId}");
            }
            //极差级别升级
            result =  await userService.ManageLevelUp();
            log.DebugFormat($"极差级别升级 ManageLevelUp:{result},orderCode:{orderCode}，buyerId：{buyerId}");
        ...
        public async Task<long> ReturnOrderAsync(long orderId)
        {
            using (MyDbContext dbc = new MyDbContext())
            {
                ...
                JournalEntity journal1 = await dbc.GetAll<JournalEntity>().SingleOrDefaultAsync(j => j.UserId == user.Id && j.OrderCode == order.Code && j.JournalTypeId == 1);
                if(journal1==null)
                {
                    return -3;
                }
                ...
                var journals = await dbc.GetAll<JournalEntity>().Where(j => j.OrderCode == order.Code && j.JournalTypeId == 1 && j.IsEnabled == false).ToListAsync();
        ...
        public async Task<long> ReturnAsync(long orderId)
        {
            using (MyDbContext dbc = new MyDbContext())
            {
                ...
                foreach (var item in orderLists)
                {
                    if (item.IsReturn == true)
                    {
                        var journals = await dbc.GetAll<JournalEntity>().Where(j =>j.GoodsId==item.GoodsId && j.OrderCode == order.Code && j.JournalTypeId == 1 && j.IsEnabled == false).ToListAsync();
