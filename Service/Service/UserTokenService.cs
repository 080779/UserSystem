...
namespace IMS.Service.Service
{
    public class UserTokenService : IUserTokenService
    {
        public async Task<long> AddAsync(long userId, string token)
        {
            ...
            {
                UserTokenEntity userToken = await dbc.GetAll<UserTokenEntity>().FirstOrDefaultAsync(u => u.UserId == userId);
                if (userToken != null)
                {
                    return -1;
                }
        ...
        public long CheckToken(long userId, string token)
        {
            using (MyDbContext dbc = new MyDbContext())
            {
                var userToken= dbc.GetAll<UserTokenEntity>().FirstOrDefault(u=>u.UserId==userId);
                if(userToken==null)
                {
                    return -1;
                }
